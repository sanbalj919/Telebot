      ...
      if(intent === expectedIntent) {
        if(intent !== "Confirmation") {
          ...
          try {
            ...
            responseMessage = buildResponse(ongoingConversation)
  ...
  function buildResponse(conversation) {
    ...
    if(conversation.entitiesFilled) {
      if(conversation.isConfirmationRequired) {
        ...
        responseText = constructResponseMessage(conversation, conversation.responses.confirmation)
      }
      else {
        console.log('%% NO %% CONFIRMATION required')
        responseText = constructResponseMessage(conversation, conversation.responses.final)
      }
    }
    else if(!conversation.entitiesFilled) {
      missingEntity = missingEntities[0]
      let missingEntityMessagesMap = new Map(conversation.responses.missingEntityPrompts)
      responseText = constructResponseMessage(conversation, missingEntityMessagesMap.get(missingEntity))
  ...
  function constructResponseMessage(conversation, response) {
    var responseMessage = ""
    if(response.type === "text") {
      responseMessage = response.message
    }
    else if(response.type === "action") {
      console.log("RESEPONSE Type is ACTION. $$ -- "+response.action)
      responseMessage = "Here are the accounts that you hold - Please type the last 4 digits of the account number for which you want to know the balance."
    }
